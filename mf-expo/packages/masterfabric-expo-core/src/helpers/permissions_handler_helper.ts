 ...
 *   expo-media-library / expo-image-picker (photo library), react-native-onesignal (notifications, iOS),
 ...
 * | notifications      | OneSignal (iOS) | POST_NOTIFICATIONS (API 33+) | |
...
import type {
 ...
 IOSInfoPlistEntry } from './permissions/types';
...
/** Android permission constants; fallback strings for web (PermissionsAndroid undefined) and older React Native where PERMISSIONS may not define them. */
const androidPerms =
  typeof PermissionsAndroid !== 'undefined' && PermissionsAndroid?.PERMISSIONS
    ? (PermissionsAndroid.PERMISSIONS as Record<string, string>)
    : null;
const ANDROID_CAMERA = androidPerms?.CAMERA ?? 'android.permission.CAMERA';
const ANDROID_READ_EXTERNAL_STORAGE = androidPerms?.READ_EXTERNAL_STORAGE ?? 'android.permission.READ_EXTERNAL_STORAGE';
const ANDROID_WRITE_EXTERNAL_STORAGE = androidPerms?.WRITE_EXTERNAL_STORAGE ?? 'android.permission.WRITE_EXTERNAL_STORAGE';
const ANDROID_READ_MEDIA_IMAGES = androidPerms?.READ_MEDIA_IMAGES ?? 'android.permission.READ_MEDIA_IMAGES';
const ANDROID_READ_MEDIA_VIDEO = androidPerms?.READ_MEDIA_VIDEO ?? 'android.permission.READ_MEDIA_VIDEO';
const ANDROID_READ_SMS = androidPerms?.READ_SMS ?? 'android.permission.READ_SMS';
const ANDROID_RECEIVE_SMS = androidPerms?.RECEIVE_SMS ?? 'android.permission.RECEIVE_SMS';
const ANDROID_SEND_SMS = androidPerms?.SEND_SMS ?? 'android.permission.SEND_SMS';
const ANDROID_BLUETOOTH_CONNECT = androidPerms?.BLUETOOTH_CONNECT ?? 'android.permission.BLUETOOTH_CONNECT';
const ANDROID_BLUETOOTH_SCAN = androidPerms?.BLUETOOTH_SCAN ?? 'android.permission.BLUETOOTH_SCAN';
 ...
 * Limited access: user chose "Selected Photos"; treat as granted — do not show "Denied".
 */
...
/** Load OneSignal Notifications API on iOS (optional peer dependency). */
function getOneSignalNotifications(): {
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports -- optional native module
    const mod = require('react-native-onesignal') as {
      OneSignal?: {
        Notifications?: {
          getPermissionAsync?: () => Promise<boolean>;
          requestPermission?: (fallbackToSettings?: boolean) => Promise<boolean>;
    ...
    if (N?.getPermissionAsync && N?.requestPermission) {
      return {
        getPermissionAsync: () => N.getPermissionAsync!(),
        requestPermission: (fallback) => N.requestPermission!(fallback),
      };
    }
    return null;
  } catch {
...
export const permissionsHandler = {
  ...
  async check(permission: PermissionType): Promise<PermissionStatus> {
    ...
    if (effective === 'notifications') {
      ...
      if (Platform.OS === 'ios') {
        const osN = getOneSignalNotifications();
        if (osN) {
          try {
            const granted = await osN.getPermissionAsync();
            return {
              granted,
              canAskAgain: true,
              status: granted ? 'granted' : 'denied',
        ...
        return unavailableStatus('react-native-onesignal required for notifications on iOS');
      }
      return unavailableStatus('notifications not supported on this platform');
    }
  ...
  async request(permission: PermissionType, _options?: PermissionOptions): Promise<PermissionStatus> {
    ...
    if (effective === 'notifications') {
      ...
      if (Platform.OS === 'ios') {
        ...
        if (osN) {
          try {
            const granted = await osN.requestPermission(false);
            return {
              granted,
              canAskAgain: true,
              status: granted ? 'granted' : 'denied',
    ...
    if (effective === 'storage') {
      ...
      // Use the system permission dialog (Allow / Don't allow) for storage; skip expo-media-library
      // here because on some devices it opens the gallery/photo picker instead.
      const apiLevel = typeof Platform.Version === 'string' ? parseInt(String(Platform.Version), 10) : (Platform.Version as number);
      const rationale = {
   ...
   * @usage permissionsHandler.requestCamera({ includePhotoLibrary: true, rationale: '...' })
   */
   ...
   * @usage permissionsHandler.requestLocation({ accuracy: 'high', background: true, rationale: '...' })
   */
   ...
   * Android uses POST_NOTIFICATIONS (API 33+); iOS uses OneSignal Notifications.requestPermission.
   * @usage permissionsHandler.requestNotifications({ alert: true, badge: true, sound: true })
   */
   ...
   * @usage permissionsHandler.requestContacts({ rationale: 'We need access to help you share with friends' })
   */
   ...
   * @usage permissionsHandler.requestBiometrics({ fallbackTitle: 'Use Passcode', promptMessage: 'Authenticate to continue' })
   */
   ...
   * @usage permissionsHandler.requestMotionFitness({ rationale: 'We need motion data to track your activity' })
   */
   ...
   * @usage permissionsHandler.requestHealth({ rationale: 'We need health data to show your progress' })
   */
   ...
   * @usage permissionsHandler.requestMediaLibrary({ accessLevel: 'readWrite', rationale: '...' })
   */
