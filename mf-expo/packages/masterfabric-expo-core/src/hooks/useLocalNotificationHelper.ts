...
export interface LocalNotificationHelperState {
  ...
  categories: { identifier: string; actions?: { identifier: string; buttonTitle: string }[] }[];
...
export function useLocalNotificationHelper(options: LocalNotificationHelperOptions = {}) {
  ...
  const [categories, setCategories] = useState<{ identifier: string; actions?: { identifier: string; buttonTitle: string }[] }[]>([]);
  ...
  useEffect(() => {
    ...
    localNotificationHelper.addActionHandler('COMPLETE', async (notification) => {
      ...
      try {
        await localNotificationHelper.cancel(notification.request.identifier);
      } catch (e) {
