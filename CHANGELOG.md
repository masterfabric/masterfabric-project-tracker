...
- **mf-go:** **GFG-179** — GraphQL **`createOrganizationProjectOrgInvite`** (host org admin/owner; optional **`capabilitiesJson`** object) and **`acceptOrganizationProjectOrgInvite`** (participant **org owner** only; **`pending` → `accepted`**). Audit events **`project_org_invite_created`** / **`project_org_invite_accepted`**. OneSignal push to participant **owner** on invite (EN/TR) when REST is configured. Accept adds participant **owner** to **`organization_project_members`** so project todo mutations work from the linked org.
- **mf-go:** **GFG-180** — GraphQL **`pendingOrganizationProjectOrgInvites`** (participant org **owner** only): pending cross-org rows with project/host names and capabilities payload for accept UI.
- **mf-expo:** **GFG-180** — **`pendingOrganizationProjectOrgInvites`** / invite / accept in **`mf-go-api.ts`**; organization **project list** **Shared** badge when project host org ≠ current org; **project detail** settings (gear): host admin/owner invite-by-org-UUID + **Todos/Purchases** capability toggles (JSON); participant **owner** pending list + **Accept** for current project. **Profile → Ortak projeler / Shared projects**: aggregates **`organizationProjects`** per member org, lists cross-host links with context + host label, opens **`/organization/[contextOrg]/project/[id]`**. EN + TR **`profile.organizations.sharedProjects*`**.
- **mf-expo:** **GFG-180** — Cross-org **invite** sheet: **Todos** / **Purchases** capability **toggles** (stored as JSON `todos` / `purchases`) instead of raw capabilities JSON. **Organization detail**: **Organization ID** on **one line** (`abc-***-************` mask + **copy** icon), **tap** copies full UUID. EN + TR.
